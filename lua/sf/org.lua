...
function M.fetch_org_list(notify)
	H.fetch_org_list(notify)
end
		...
		if choice ~= nil then
			vim.fn.jobstart("sf config set target-org " .. choice, {
				on_exit = function(_, code)
					if code == 0 then
						S.target_org = choice
						vim.notify("Local target_org set: " .. choice, vim.log.levels.INFO)
					else
...
H.store_orgs = function(data)
	...
	for _, v in pairs(scratch_org_data) do
		if v.isDefaultUsername == true then
			S.target_org = v.alias
		end

		if v.isExpired == false then
			table.insert(H.orgs, v.alias)
		end
	end
...
H.fetch_and_store_orgs = function(notify)
	vim.fn.jobstart("sf org list --json", {
		stdout_buffered = true,
		on_stdout = function(_, data)
			H.store_orgs(data)
			if notify == true then
				vim.notify("Fetched updated org list!", vim.log.levels.INFO)
			end
		end,
	})
end

H.fetch_org_list = function(notify)
	U.is_sf_cmd_installed()
	H.clean_org_cache()
	if notify == true then
		vim.notify("Fetching org list...", vim.log.levels.INFO)
	end
	H.fetch_and_store_orgs(notify)
end
...
H.retrieve_metadata_lists = function()
	...
	vim.notify("Retrieving metadata from " .. S.target_org, vim.log.levels.INFO)
